...
function CollapsibleSteps({
  ...
  const verbosity = useAtomValue(toolVerbosityAtom)
  const isMinimal = verbosity === "minimal"
  const [isExpanded, setIsExpanded] = useState(defaultExpanded || verbosity === "expanded")
  ...
  return (
    <div className="mb-2" data-collapsible-steps="true">
      <div
        className={cn(
          "flex items-center justify-between rounded-md py-0.5 px-2 transition-colors",
          !isMinimal && "cursor-pointer hover:bg-muted/50",
        )}
        onClick={() => !isMinimal && setIsExpanded(!isExpanded)}
      >
