...
function ProjectDetail({ projectId }: { projectId: string }) {
  ...
  // Local state
  const [saveTarget, setSaveTarget] = useState<"cursor" | "2code">("2code")
  ...
  useEffect(() => {
    if (configData) {
      const newSaveTarget = configData.source === "cursor" ? "cursor" : "2code"
          ...
          <div className="bg-background rounded-lg border border-border overflow-hidden">
            <div className="flex items-center justify-between p-4">
              ...
              <Select
                value={saveTarget}
                onValueChange={(v) => {
                  setSaveTarget(v as "cursor" | "2code")
                  pendingSaveRef.current = true
                }}
              >
                <SelectTrigger className="w-auto px-3">
                  <span className="text-sm font-mono">
                    {saveTarget === "cursor" ? ".cursor/worktrees.json" : ".2code/worktree.json"}
                  </span>
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="2code">.2code/worktree.json</SelectItem>
                  {cursorExists && (
