...
export async function auditAllZones(
  ...
  // Emit initial progress so the frontend knows the audit has started
  ...
  for (let i = 0; i < zones.length; i++) {
    ...
    // Check if this zone needs a profile generated (first-time audit)
    let zoneNeedsProfile = false
    if (!profileId) {
      const existingProfile = db.select().from(auditProfiles)
        .where(and(
        ...
        .find(p => {
          try {
            ...
            return Array.isArray(ids) && ids.includes(zone.id)
          } catch { return false }
        })

      if (!existingProfile) {
        zoneNeedsProfile = true
        progress[i].status = "profiling"
        onProgress?.(progress, runId)
