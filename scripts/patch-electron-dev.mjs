// Patches the Electron.app bundle in node_modules to show "2Code" name and icon in macOS dock during dev mode.
...
if (existsSync(plistPath)) {
  try {
    execSync(`/usr/libexec/PlistBuddy -c "Set :CFBundleName 2Code" "${plistPath}"`)
    execSync(`/usr/libexec/PlistBuddy -c "Set :CFBundleDisplayName 2Code" "${plistPath}"`)
    console.log("[patch-electron-dev] Updated Info.plist: name -> 2Code")
  } catch (e) {
